...
public class BruteForceAI extends ComputerAI{
	...
	public static int maxCheckedNodes = 4000;
	public static int leeway=200;
	public static int nodesArraySize=maxCheckedNodes+leeway;
	public static int maxOptions=50;
	
	public static int checkedNodes;
	
	private static BruteForceAI instance;
	
	public static BruteForceAI getInstance(){
		if(instance==null){
			instance=new BruteForceAI();
		}
		return instance;
	}
	
	//The first stack checks the first option, the second checks the second, and so on
	@SuppressWarnings("unchecked")
	private LinkedList<Node>[] nodeStacks=new LinkedList[maxOptions];

	//This stack is used to add the nodes to the finalScoreCalculationQueue in the right order
	...
	private void createPlan(Battle battle) {
		checkedNodes=0;
		Node firstNode=Node.getNewNode(battle, -1, -1, 0, "");
		...
		/*debugPrintFinalScore(firstNode);
		System.out.println();*/
	...
	private void debugPrintFinalScore(Node node) {
		System.out.println(node.debuggingName+" P:"+node.scenario.decidingPlayer+"   ini:"+node.initialScore+"   fin:"+node.finalScore);
		if(!node.continuations.isEmpty()){
			for(Node continuation : node.continuations){
	...
	private void debugPrintNode(Node node) {
		...
		System.out.println(node.debuggingName+"        St:"+node.scenario.state+"     Tr:"+node.passedTurns+"      Ch: "+choice);
	...
	private void expandNode(Node node) {
		...
		if(node.scenario.state==Battle.choosingActiveEffectState) {
			...
			if(node.scenario.choosingTargetStateAction.type==Battle.invAction || node.scenario.choosingTargetStateAction.type==Battle.dscAction) {
		...
		for(int c : node.scenario.zones[node.scenario.turn][zone]){
			...
			for(Effect effect : card.model.effects){
				if(card.turn && effect.zone==zone && effect.sequence.isEmpty()) {
					...
					Node newNode=Node.getNewNode(newScenario, card.battleId, effectNumber, node.passedTurns, node.debuggingName+node.continuations.size());
	...
	private void continuePassingTurn(Node node) {
		...
		Node newNode=Node.getNewNode(newScenario, -1, -1, node.passedTurns+1, node.debuggingName+node.continuations.size());
		node.continuations.addLast(newNode);
	}
	...
	private void continueWithTargetFrom(Node node, int player, int zone){
		for(int c : node.scenario.zones[player][zone]){
			...
			Node newNode=Node.getNewNode(newScenario, c, -1, node.passedTurns, node.debuggingName+node.continuations.size());
			node.continuations.addLast(newNode);
		}
	}
	...
	private void continueWithNoTarget(Node node) {
		...
		Node newNode=Node.getNewNode(newScenario, -1, -1, node.passedTurns, node.debuggingName+node.continuations.size());
		node.continuations.addLast(newNode);
	}
